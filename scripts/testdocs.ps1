...
Write-Host 'Running "prepdocs.py " {$env:AZURE_ADLS_GEN2_STORAGE_ACCOUNT}'
$cwd = (Get-Location)
...
if ($env:AZURE_ADLS_GEN2_STORAGE_ACCOUNT) {
  $adlsGen2StorageAccountArg = "--datalakestorageaccount $env:AZURE_ADLS_GEN2_STORAGE_ACCOUNT --datalakekey $env:AZURE_ADLS_GEN2_STORAGE_KEY"
  $adlsGen2FilesystemPathArg = ""
  if ($env:AZURE_ADLS_GEN2_FILESYSTEM_PATH) {
    $adlsGen2FilesystemPathArg = "--datalakepath $env:ADLS_GEN2_FILESYSTEM_PATH"
  }
...
$argumentList = "./scripts/test.py `"$cwd/data/*`" $adlsGen2StorageAccountArg $adlsGen2FilesystemArg $adlsGen2FilesystemPathArg $searchAnalyzerNameArg " + `
