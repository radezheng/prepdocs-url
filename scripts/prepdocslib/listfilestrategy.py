...
class File:
    ...
    def __init__(self, content: IO = None, file_url: str = None, acls: Optional[dict[str, list]] = None):
...
#write a AzureBlobListFileStrategy class that inherits from ListFileStrategy
#and implements the list_paths and list methods
class AzureBlobListFileStrategy(ListFileStrategy):
    """
    Concrete strategy for listing files that are located in a storage account
    """
    ...
    async def list(self) -> AsyncGenerator[File, None]:
        ...
        if self.verbose:
            print(f"Listing files in container '{self.container}' with prefix '{self.path}'")
        try:
            async for blob in container_client.walk_blobs(name_starts_with=self.path):
                
                file_url = f"https://{self.storage_account}.blob.core.windows.net/{self.container}/{blob.name}"
                if self.verbose:
                    print(f"Found file '{file_url}'")
                yield File(file_url=file_url)
        except Exception as e:
            if self.verbose:
                print(f"An error occurred: {e}")
            # Handle the exception or re-raise it
        finally:
            # Clean up the clients if necessary
...
class ADLSGen2ListFileStrategy(ListFileStrategy):
    ...
    async def list(self) -> AsyncGenerator[File, None]:
        async with DataLakeServiceClient(
            ...
            async for path in self.list_paths():
                #get file url
                file_url = f"https://{self.data_lake_storage_account}.dfs.core.windows.net/{self.data_lake_filesystem}/{path}"
                ...
                try:
                    ...
                    yield File(file_url=file_url, acls=acls)
